...
- A terminal `read ETIMEDOUT` from a required Oracle leaf ends the current loop invocation. This applies to `oracle-issue-plan` and to `oracle-pr-review` only when review-marker recovery cannot prove publication. `oracle-pr-feedback-plan` is not invoked by the loop, so its standalone failure cannot block loop progress.
...
For a terminal `read ETIMEDOUT` from a required Oracle leaf, explicitly report whether the failure came from Issue planning or from PR review and whether review-marker recovery succeeded. Do not report `oracle-pr-feedback-plan` transport failures as loop blockers because the loop does not invoke that optional standalone skill.
